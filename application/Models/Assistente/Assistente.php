...
namespace Agencia\Close\Models\Assistente;
...
class Assistente extends Model
{
    ...
    {
        $this->create = new Create();
        $this->create->ExeCreate('assistentes', $data);
        return $this->create;
    }
    ...
    /**
     * Retorna lista distinta de credenciais para popular Select2 com tags
     */
    public function getCredenciaisDistinct(int $empresa): Read
    {
        $this->read = new Read();
        $this->read->FullRead(
            "SELECT DISTINCT credencial
             FROM assistentes
             WHERE empresa = :empresa
               AND credencial IS NOT NULL
               AND credencial <> ''
    ...
    /**
     * Busca assistentes para DataTables (Server-Side Processing)
     */
    public function getAssistentesDataTable(int $empresa, array $params, array $filtros = []): array
    {
        ...
        if (!empty($search)) {
            $searchWhere .= " AND (
                nome LIKE :search OR
                profissao LIKE :search OR
                cidade_estado LIKE :search OR
                telefone_contato LIKE :search
            )";
            $searchParams .= "&search=%{$search}%";
        }

        $baseQuery = "SELECT * FROM assistentes";
